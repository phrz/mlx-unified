...
class ModelProvider:
    def __init__(self, cli_args: argparse.Namespace):
        ...
        self.delegate = None
        self.moe_session = None  # mlx-moe delegation (--stream-experts)
        self.moe_executor = None
    ...
    def _load(self, model_path, adapter_path=None, draft_model_path=None):
        ...
        if getattr(self.cli_args, "stream_experts", False):
            # Expert-aware SSD streaming: DELEGATED to mlx-moe's own Server
            # session (pinned in [vision]) — its load/refine/generate pipeline is
            # validated end-to-end, and it runs single-threaded, so everything
            # (load included) happens on ONE dedicated worker thread; our HTTP
            # threads only adapt the wire (see _serve_streamed_experts). Mirrors
            # the vlm_delegate pattern for diffusion families.
            if self.is_distributed or adapter_path is not None:
                raise ValueError(
                    "--stream-experts is incompatible with distributed mode and adapters."
                )
            try:
                from mlx_moe.server import Server as _MoeSession
            ...
            executor = ThreadPoolExecutor(
                max_workers=1, thread_name_prefix="moe-stream"
            )
            executor.submit(session.load).result()
            self.moe_session = session
            self.moe_executor = executor
            model, tokenizer = session._model, session._tokenizer
...
class ResponseGenerator:
    ...
    def _serve_single(self, request):
        ...
        try:
            ...
            if getattr(self.model_provider, "delegate", None) is not None:
                self._serve_delegated_vlm(rqueue, request, args)
                return
            ...
            for gen in stream_generate(
                ...
                draft_kind=getattr(self.model_provider, "draft_kind", None),
            ...
            if point_text:
                ...
                if rendered and not ctx._should_stop:
                    rqueue.put(Response(rendered, gen.token, 0.0, None, ()))
    ...
    def _serve_streamed_experts(self, rqueue, request, args):
        """Serve one request through the mlx-moe session (mlx-unified).

        The generation loop — keyed KV-prefix reuse, adaptive expert-cache
        maintenance, drain rounds — is mlx-moe's own Server._stream, executed on
        the session's single dedicated thread (the engine is validated
        single-threaded; this HTTP server is thread-per-request). This method
        adapts the wire only: tokenize with our machinery, map sampling, and
        forward stream responses. Raw text passthrough (thinking tags included)
        like the diffusion delegate; no local prompt cache — the session owns
        KV reuse per its exact-prefix protocol.
        """
        session = self.model_provider.moe_session
        executor = self.model_provider.moe_executor
        tokenizer = self.model_provider.tokenizer

        prompt, _, _, initial_state = self._tokenize(tokenizer, request, args)
        _, text_sm = self._make_state_machine(
            self.model_provider.model_key,
            tokenizer,
            args.stop_words,
        )
        ...
        # Per-conversation KV key, like mlx-moe's own handlers (an explicit
        # client cache_key/session_id, else a hash of the conversation's first
        # user message): unrelated conversations get FRESH prompt caches instead
        # of exercising the cross-conversation trim path with a shared key —
        # which measurably bled prior context into new requests.
        ...
        def consume_mtp():
            ...
            def on_tokens(toks):
                for t in toks:
                    detok.add_token(t)
                    rqueue.put(
                        Response(detok.last_segment, t, 0.0, None, ())
                    )
    ...
    def _serve_delegated_vlm(self, rqueue, request, args):
        """Serve one request from a delegated VLM family (mlx-unified).

        Prompt rendering, preprocessing, and generation all run inside
        mlx-vlm (see vlm_delegate), so model-specific caches and generation
        modes remain authoritative there. This method adapts only the wire.
        Diffusion families additionally support opt-in draft-block chunks."""
        ...
        ctx = GenerationContext(
            ...
            has_thinking=tokenizer.has_thinking,
            tool_parser=tokenizer.tool_parser,
            text_sm=make_text_state_machine(tokenizer, args.stop_words),
            initial_state=initial_state,
            prompt=prompt,
        )
        rqueue.put(ctx)
        ...
        try:
            for gen in response:
                ...
                if gen.finish_reason == "stop":
                    # Some delegated engines attach a legitimate final decoded
                    # segment to the terminal result. Feed it through the same
                    # control matcher before discarding only the still-pending
                    # stop-marker prefix. Ordinary EOS/stop text remains
                    # stripped because it is registered in the state machine.
