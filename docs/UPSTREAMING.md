...
| Server `response_format` enforcement (`json_schema` / `json_object`) with outlines-core: `structured_output.py` compiles the schema to a token automaton over the served vocabulary and masks logits per step (batched, sequential, and delegated mlx-vlm lanes; thinking blocks pass through; bad schemas 400) | `structured-output` branch | pending | Upstream has no constrained decoding at all; outlines-core is what vLLM/SGLang build on, so the shape should be familiar. Optional extra (`mlx-lm[structured]`). |
